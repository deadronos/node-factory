...
export const useGameStore = create<GameStore>()(
  persist(
    ...
    {
      name: 'node-factory-storage',
      storage: createJSONStorage(() => localStorage),
      // Handle the Set in research manually during serialization
      // Convert Set to Array for JSON.stringify
      // Handle the conversion back to Set during hydration
      onRehydrateStorage: () => (state) => {
        if (state && state.research && Array.isArray(state.research.unlockedNodes)) {
          state.research.unlockedNodes = new Set(state.research.unlockedNodes);
        }
      },
    }
  )
