...
interface ResearchPanelPresentationalProps {
  researchUnlockedNodes: Set<NodeType>;
  economyModules: number;
  onSpend: (cost: number) => boolean;
  onUnlockNode: (nodeType: NodeType) => void;
}

export function ResearchPanelPresentational({
  researchUnlockedNodes,
  economyModules,
  onSpend,
  onUnlockNode,
}: ResearchPanelPresentationalProps) {
  const handleUnlock = (nodeType: NodeType) => {
    const cost = RESEARCH_COSTS[nodeType];
    if (onSpend(cost)) {
      onUnlockNode(nodeType);
    } else {
  ...
  const lockedNodes: NodeType[] = ['splitter'].filter(
    (type) => !researchUnlockedNodes.has(type as NodeType)
  ) as NodeType[];
